...
const ImageAccordion = ({
  ...
  }, [activeValue, hovered, data])
  return (
    <div className="rb-row md:items-start">
      <div
      ...
      <div className="w-full md:w-5/12 flex-col items-start flex">
        <div className="grid grid-cols-1 grid-rows-1 md:pl-7">
          {/* {data.map(({ key, image }) => (
            <img
              key={key}
              alt=""
          ...
          <img alt="" {...image} className="w-full col-start-1 row-start-1 md:mt-[10px]" />
        </div>
      </div>
    </div>
  )
  ...
  ({ children, className, index, customClass, ...props }, forwardedRef) => (
    <Header className="flex !font-opensans relative">
      <Trigger
        ...
        ref={forwardedRef}
      >
        <span className="inline-flex flex-wrap group-data-[state='open']:opacity-100 opacity-50">
