...
import {
  ...
  fileSizeValidation,
  // liveUrlValidation,
  phoneNumberValidation, isValidName, companyNameValidation, sanitizeformData,
  validateDescription,
  isValidString
} from '@/components/form/utils'
import { Button } from '@/components/ui'
...
const schema = object({
  ...
  Company: string().required('Company name is required').test('name', 'Company name is invalid', companyNameValidation), //company
  // Website: string()
  //   .test('website', 'Website url not valid', liveUrlValidation)
  //   .required('Website is required'),
  Description: string().test('name', 'Description is invalid', validateDescription),
  theFile: mixed().test('filesize', 'File size not more than 2 MB', (v) =>
    fileSizeValidation(v)
  ),
  Email_Opt_Out: bool(),
    ...
    <div>
      {
        isBlogRoute ?
          ...
          :
          <p className={`text-rb-black/90 font-normal ${isPopop ? 'text-xs text-center mt-2 mb-3' : 'text-sm md:text-2xl md:max-w-[80%] md:mt-4 mb-6 md:mb-21'}`}>
            {!content ? ( 
              <p>We are always looking to work on exciting briefs.<br /> Let&apos;s start a conversation today!</p> 
              ) : (
        ...
        >
          ...
          <input
            ...
            defaultValue={
              ...
              window.location.hostname.includes('staging')
                ? 'https://www.staging.b2c.redbangle.com/success'
                : 'https://www.b2c.redbangle.com/success'
            }
          ...
          <div className={`${isBlogRoute ? '' : 'md:gap-y-14'} grid grid-cols-1 md:grid-cols-2 gap-6`}>
            ...
            <PhoneInput
              name="Phone"
              label={"Mobile"}
              placeholder={isBlogRoute ? 'Mobile Number' : "Your mobile number"}
              required
              borderedInput={isBlogRoute}
            />
            ...
            <div className="md:col-span-2">
              {
                isBlogRoute ? '' :
                  <Label>
                    {!content ? "Tell us about the project (Scope, Timeline, Budget):" : "Send us a query"}
                  </Label>
              }
              {
                isBlogRoute ?
