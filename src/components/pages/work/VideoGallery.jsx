...
export const VideoGallery = ({ sources = [], type = null,showHeading = true  }) => {
  ...
  return (
    <div id='video' className={classNames('container', { 'relative': type === 'videogallery' })}>
        {showHeading && <LineHeading className="mb-6 md:!mb-9 md:mt-20  mt-9">Check out the videos</LineHeading> }
      <div
        className={`bg-rb-black ${styles.playerwrapper} ${type === 'videogallery' ? 'mt-1 md:mt-6' : 'mt-0 md:mt-0'}`}
        data-rb-cursor
...
{(sources.length > 1) && (
      <div>
        ...
        <Swiper
          ...
          spaceBetween={12}
          slidesPerView={2.1}
          className={classNames('swiper-overflow-visible', { 'md:w-[100%]': type === 'videogallery' })}
          breakpoints={{
            768: {
              slidesPerView: 3,
              spaceBetween: 24,
