...
const Articles = ({ featuredPost, posts: { edges, pageInfo }, setisPopupOpen }) => {
  ...
  const loadMorePosts = () => {
    ...
    sessionStorage.setItem('blogInd', Math.min(nextVisiblePosts, filteredPosts.length));
  };
  ...
  const handleCategoryChange = (category) => {
    ...
    setVisiblePosts(9); // reset visible count when filtering
  };
  ...
  return (
    <>
      <SEO
        title="Red Bangle - Driving growth for B2C brands through creativity and AI"
        description="Discover the latest trends in culture, content and AI. Get insights that keep your brand ahead of the game."
      />
        ...
        <div className="container">
          ...
          <div className="flex flex-wrap lg:flex-nowrap gap-8 items-center mb-13 border-b border-b-rb-davy-grey pb-13">
            <p className="flex-1 md:text-[32px] font-semibold tracking-[-1.28px] mb-0">
              Discover the latest trends in branding and culture. Gain valuable insights that
              keep your brand on top of the game.
            </p>
        ...
        <div className="container">
          ...
          <div id='discover' className="grid md:grid-cols-2 lg:grid-cols-3 gap-y-16 gap-x-8 pt-8">
            {filteredPosts.slice(0, visiblePosts).map(({ date, author, featuredImage, primaryCategory, slug, categories, title }) => (
              ...
              <div key={slug}>
                ...
                <a
                  href={`/blog/${slug}`}>
                  <div className='lg:relative h-28 lg:h-[100px] lg:mb-5'>
                    {/* <div className="flex gap-2 mb-4">
          ...
          {filteredPosts.length > 6 && visiblePosts >= filteredPosts.length && (
            <Button
              ...
              onClick={() => {
                setVisiblePosts(9); // or set to 6 if that's your default
