...
const ArticleSingle = ({ article, relatedArticle, tocTree, setisPopupOpen }) => {
  ...
  useEffect(() => {
    ...
    if (article?.title === 'The Sound of Success: How Sonic Branding Hits All the Right Notes') {
      const audioElements = [
          ...blogRef.current?.querySelectorAll('li audio'),
          ...blogRef.current?.querySelectorAll('div audio')
      ];
      const paragraphs = [...blogRef.current?.querySelectorAll('p')];
      const spans = blogRef.current?.querySelectorAll('div span');
      const liTag = blogRef.current?.querySelectorAll('li');
      const olTag = blogRef.current?.querySelectorAll('ol');
      const sonic_title = blogRef.current?.querySelectorAll('.sonic-title');

      olTag.forEach((o)=>{
        o.style.listStyle = 'auto';
        o.style.marginLeft = '22px';
      })

      sonic_title.forEach((s)=>{
        s.style.textAlign = 'center';
        s.style.marginBottom = '10px';
      })

      liTag.forEach((l)=>{
        const paragraph = l.querySelector('p');
        ...
        if (paragraph) {
          ...
          paragraph.style.bottom = '8px'
          paragraph.style.marginLeft = '1.4rem'
        }
      ...
      audioElements.forEach((audio, index) => {
          ...
          if (audio && !audio.previousElementSibling?.classList.contains('audio-control-icon')) {
              audioRefs.current[index] = audio;
  
              audio.style.display = 'none';
              const img = document.createElement('img');
              img.src = '/img/audio_icon.svg'; 
              img.alt = 'Sound Icon';
              img.className = 'w-6 ml-1 cursor-pointer audio-control-icon inline-audio inline relative top-2';
              ...
              parentElement.insertBefore(img, audio);
          }
      });
  }
